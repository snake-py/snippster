export function snippetReducer(state = {}, action) {
  switch (action.type) {
    ...
    case 'EDIT_TITLE':
      ...
      return {
        ...state,
        snippets: state.snippets.map((snippet) => (action.payload.snippet.id === snippet.id ? { ...snippet, title: action.payload.title, active: true,  isSaved: false } : { ...snippet, active: false })),
    ...
    case 'EDIT_DESCRIPTION':
      ...
      return {
        ...state,
        snippets: state.snippets.map((snippet) => (action.payload.snippet.id === snippet.id ? { ...snippet, description: action.payload.description, active: true,  isSaved: false } : { ...snippet, active: false })),
      ...
      case 'EDIT_CODE':
        ...
        case 'ADD':
          ...
          return {
            ...state,
            snippets:  [...state.snippets.map((snippet) => ({ ...snippet, active: false}) ), {id:'new', description: '', code: '', title: '', language: '', framework: '', isSaved: false}],
            activeSnippet: {id:'new', description: '', code: '', title: '', language: '', framework: '', isSaved: false}
          };
    default:
      return state;
  }
